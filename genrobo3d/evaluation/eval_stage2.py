...
from genrobo3d.train.train_stage2_dit import MODEL_FACTORY
...
# from rlbench.backend.exceptions import IKError, ConfigurationPathError, InvalidActionError
...
def evaluate_simple_actioner(env, task_str, variation, actioner, num_episodes=10, max_steps=30):

    success_count = 0
    for demo_id in range(num_episodes):
        ...
        try:
            ...
            print(f"Demo {demo_id} reset error: {e}")
            continue
        ...
        for step_id in range(max_steps):
            ...
            try:
                ...
                print(f"Error in demo {demo_id}, step {step_id}: {e}")
                break

    success_rate = success_count / num_episodes
    print(f"Task: {task_str}+{variation} | Success Rate: {success_rate:.2f}")
    return success_rate
...
from genrobo3d.rlbench.environments_joint import RLBenchEnv
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ptv3_ckpt', type=str)
    parser.add_argument('--dit_ckpt', type=str)
    parser.add_argument('--taskvar', type=str)
    parser.add_argument('--microstep_data_dir', type=str)
    ...
    parser.add_argument('--save_obs_outs_dir', type=str, default=None)
    parser.add_argument('--real_robot', action='store_true')
    parser.add_argument('--best_disc_pos', type=str, default='max')
    parser.add_argument('--num_ensembles', type=int, default=1)
    parser.add_argument('--remained_args', nargs='*', default=[])
    args = parser.parse_args()

    if args.exp_config is None:
        print("No command-line arguments provided. Using default config values.")
        # for task in ('open_box+0'): # , 'close_laptop_lid+0', 'close_fridge+0', 'push_button+0'):
        ...
        dit_ckpt_path = 'data/experiments/gembench/stage2_dit_0617/ckpts/model_step_11000.pt'
        dit_cfg_path = "data/experiments/gembench/stage2_dit_0617/logs/training_config.yaml"

        args.exp_config = dit_cfg_path
        args.checkpoint = dit_ckpt_path
        args.ptv3_ckpt = ptv3_ckpt_path
        args.dit_ckpt = dit_ckpt_path
        args.taskvar = 'put_groceries_in_cupboard+3'
        args.microstep_data_dir = 'data/gembench/test_dataset/microsteps/seed200'
        args.num_demos = 20
        args.headless = False
        args.image_size = [256, 256]
        args.cam_rand_factor = 0.0
        args.max_steps = 30
        args.device = 'cuda'
        args.save_obs_outs_dir = None
        args.real_robot = False
        args.best_disc_pos = 'max'
        ...
        # Load configs
        ptv3_cfg = get_config(args.exp_config, [])
        dit_cfg = get_config(args.exp_config, [])

        # Load models
        ptv3_model = SimplePolicyPTV3CA(ptv3_cfg.MODEL)
        ptv3_ckpt = torch.load(args.ptv3_ckpt, map_location=device)
        ...
        simple_actioner = SimpleActioner(dit_model, ptv3_model, full_actioner, device=device)

        # Parse task + variation
        ...
        # Run evaluation
        success_rate = evaluate_simple_actioner(
            env,
            task_str,
            variation,
            simple_actioner,
        ...
        result_path = os.path.join('results_eval', f'{task_str}+{variation}.json')
