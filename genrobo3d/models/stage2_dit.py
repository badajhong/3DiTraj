...
class DiTraj(BaseModel):
    ...
    def forward(self, batch, pc_tokens, final_pred_joints_actions, max_steps, batch_size, robot_type, compute_loss=False, **kwargs):
        ...
        # Separate normalized joint angles and gripper values
        norm_joint = norm_pred_traj[:, :7]   # shape: (batch * step, 7)
        gripper = norm_pred_traj[:, 7:]      # shape: (batch * step, 1)

        # Define Franka Panda joint limits
        joint_mins = torch.tensor([-2.8973, -1.7628, -2.8973, -3.0718, -2.8973, -0.0175, -2.8973], dtype=norm_pred_traj.dtype, device=norm_pred_traj.device)
        joint_maxs = torch.tensor([ 2.8973,  1.7628,  2.8973, -0.0698,  2.8973,  3.7525,  2.8973], dtype=norm_pred_traj.dtype, device=norm_pred_traj.device)

        # Rescale joint limits
        pred_joint = 0.5 * (norm_joint + 1) * (joint_maxs - joint_mins) + joint_mins  # shape: (batch * step, 7)

        # gripper openness in [-10, 10]
        pred_gripper = 10 * gripper

        pred_traj = torch.cat([pred_joint, pred_gripper], dim=-1)  # shape: (batch * step, 8)        
