...
class DiTraj(BaseModel):
    ...
    def forward(self, batch, pc_tokens, final_pred_joints_actions, max_steps, batch_size, robot_type, compute_loss=False, **kwargs):
        ...
        final_pred_joints_actions = torch.concat((batch['gt_joints'][end_indices], batch['gt_actions'][end_indices][:, -1:]), dim=1)  # (batch, 8)
    ...
    def ddpm_sample(self, batch, pc_tokens, final_pred_joints_actions, max_steps, batch_size, robot_type, compute_loss=False, **kwargs):
        ...
        last_index = torch.tensor([len(batch['step_ids']) - 1], device=batch['step_ids'].device)
        end_indices = torch.cat([end_indices, last_index])
        final_pred_joints_actions = torch.concat((batch['gt_joints'][end_indices], batch['gt_actions'][end_indices][:, -1:]), dim=1)
