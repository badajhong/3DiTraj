...
class ActionHead(nn.Module):
    ...
    def forward(
        ...
        # only differentiable cases: max, multiscale_max -> non differentiable
        if self.reduce == 'mean':
        ...
        xo = action_embeds[..., -1]
        xo = torch.tanh(xo) * 10 # Avoid inf in BCEwithLogits
...
class SimplePolicyPTV3AdaNorm(BaseModel):
    ...
    def forward(self, batch, compute_loss=False, global_step=None, **kwargs):
        '''batch data:
        ...
        pc_token = point_outs[-1].feat   # (N= num of points in batch, C= num of points' features)
        pc_coord = point_outs[-1].coord  # (N= num of points in batch, 3= coordinate(x,y,z))

        # Predict the initial task space pc_token -> final gt_action 
        ...
        start_idx = 0
        for i, n in enumerate(batch['npoints_in_batch']):
            end_idx = start_idx + n
            if i in start_indices:
                new_pc_token_list.append(pc_token[start_idx:end_idx])
                new_pc_coord_list.append(pc_coord[start_idx:end_idx])
            start_idx = end_idx

        new_pc_token = torch.cat(new_pc_token_list, dim=0)
        new_pc_coord = torch.cat(new_pc_coord_list, dim=0)

        new_npoints_in_batch = [batch['npoints_in_batch'][i] for i in start_indices]

        pred_actions = self.act_proj_head(
            new_pc_token, new_npoints_in_batch, coords=new_pc_coord,
        ...
        # quaternion normalization #TODO? Is this fine?
        pred_rot = pred_rot / pred_rot.norm(dim=-1, keepdim=True).clamp(min=1e-8)

        final_pred_actions = torch.cat([pred_pos, pred_rot, pred_open.unsqueeze(-1)], dim=-1)
        
        if compute_loss:
            # Compute loss of end trajectory only
            ...
            if use_teacher:
                tgt_pos = batch['gt_actions'][end_indices][..., :3]
                tgt_rot = batch['gt_actions'][end_indices][..., 3:-1]
                joint_input = torch.cat([tgt_pos, tgt_rot], dim=-1)
            else:
                joint_input = torch.cat([pred_pos, pred_rot], dim=-1)

            norm_pred_joint = self.ik_mlp(joint_input)

            joint_mins = torch.tensor([-2.8973, -1.7628, -2.8973, -3.0718, -2.8973, -0.0175, -2.8973], device=norm_pred_joint.device)
            joint_maxs = torch.tensor([ 2.8973,  1.7628,  2.8973, -0.0698,  2.8973,  3.7525,  2.8973], device=norm_pred_joint.device)

            # Scale tanh output from [-1,1] to [min, max]
            pred_joint = 0.5 * (norm_pred_joint + 1) * (joint_maxs - joint_mins) + joint_mins

            final_pred_joints_actions = torch.cat([pred_joint, pred_open.unsqueeze(-1)], dim=-1)

            losses = self.compute_loss(
                pred_actions, batch['gt_actions'][end_indices], pred_joint, batch['gt_joints'][end_indices],
            )
            return final_pred_actions, final_pred_joints_actions, new_pc_token_list, new_pc_coord_list, losses
        else:
            pred_joint = self.ik_mlp(torch.cat([pred_pos, pred_rot], dim=-1))
            final_pred_joints_actions = torch.cat([pred_joint, pred_open.unsqueeze(-1)], dim=-1)

            return final_pred_actions, final_pred_joints_actions, new_pc_token_list, new_pc_coord_list

    def compute_loss(self, pred_actions, tgt_actions, pred_joint, tgt_joint):
        """
        Args:
        ...
        total_loss = self.config.loss_config.pos_weight * pos_loss + \
                     ...
                     self.config.loss_config.joint_weight * joint_loss + \
                     self.config.loss_config.open_weight * open_loss
        
        return {
            'pos': pos_loss, 'rot': rot_loss, 'joint': joint_loss, 'open': open_loss,
            'total': total_loss
        }
...
class SimplePolicyPTV3CA(SimplePolicyPTV3AdaNorm):
    ...
    """
    def __init__(self, config):
        super().__init__(config)  # BaseModel.__init__(self)=> try to bring SimplePolicyPTV3AdaNorm
    ...
    def prepare_ptv3_batch(self, batch):
        # print("🟢 prepare_ptv3_batch FROM:", self.__class__.__name__)
        outs = {
            'coord': batch['pc_fts'][:, :3],
            'grid_size': self.config.action_config.voxel_size,
            'offset': batch['offset'],
