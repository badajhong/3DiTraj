...
from rlbench.action_modes.arm_action_modes import EndEffectorPoseViaPlanning, JointPosition, JointTorque
...
class Mover:
    def __init__(self, task: TaskEnvironment, disabled: bool = False, max_tries: int = 200):
        self._task = task
        self._step_id = 0
        self._max_tries = max_tries
        self._disabled = disabled
        self._tolerance = 1e-2  # High precision
    ...
    def __call__(self, action: np.ndarray, verbose=True):
        ...
        # Apply action and wait for exact joint convergence
        ...
        for attempt in range(self._max_tries):
            ...
            current_joints = self._task._robot.arm.get_joint_positions()
            # print ('current_joints:', current_joints)
            error = np.abs(np.array(current_joints) - target_joints)

            if verbose:
                print(f"[Step {self._step_id}] Try {attempt+1}: Error = {error}")

            if np.all(error < self._tolerance):
                break

        # # Final check for precision
        # final_joints = self._task._robot.arm.get_joint_positions()
        # if not np.allclose(final_joints, target_joints, atol=self._tolerance):
        #     print("Joint convergence failed after", self._max_tries, "steps.")
        #     print("Final error:", np.abs(np.array(final_joints) - target_joints))
        #     raise RuntimeError("Joints did not reach target")
