...
class ptv3_dit_PolicyDataset(Dataset):
    def __init__(
            self, data_dir, joint_data_dir, instr_embed_file, taskvar_instr_file, taskvar_file=None,
        ...
        for taskvar in self.taskvars:
            if not os.path.exists(os.path.join(data_dir, taskvar)):
                assert False, f"Directory or file {os.path.join(data_dir, taskvar)} does not exist."
            ...
            json_path = os.path.join(joint_data_dir, f"{taskvar}.json")
            if not os.path.exists(json_path):
                assert False, f"Joint data JSON file not found: {json_path}"
    ...
    def __getitem__(self, idx):
        ...
        episode_key = data_id.decode("utf-8")
        episode_data = self.joint_data[taskvar][taskvar][episode_key]
        joint_data = [step["joint_positions"] for step in episode_data]

        outs = {
            'data_ids': [], 'pc_fts': [], 'step_ids': [],
            'pc_centroids': [], 'pc_radius': [], 'ee_poses': [], 
            'txt_embeds': [], 'gt_actions': [], 'gt_joints': []
        }
        ...
        for t in range(num_steps):
            ...
            # joint per steps
            # if t >= len(joint_data):
            #     raise IndexError(f"Invalid timestep t={t} for joint_data of length {len(joint_data)}")
            # print(f"[DEBUG] joint_data len: {len(joint_data)}, trying to access index: {t}")
            # gt_joint = np.array(joint_data[t])
            ...
            outs['gt_joints'].append(torch.from_numpy(gt_joint).float())
...
def base_collate_fn(data):
    ...
    for key in ['pc_fts', 'ee_poses', 'gt_actions' 'gt_joints']:   
...
def ptv3_collate_fn(data):
    ...
    for key in ['ee_poses', 'gt_actions', 'gt_joints']:   
        batch[key] = torch.stack(batch[key], 0)
...
if __name__ == '__main__':
    ...
    dataset = ptv3_dit_PolicyDataset(
        'data/gembench/train_dataset/keysteps_bbox_pcd/seed0/voxel1cm',
        'data/gembench/train_dataset/keystep_joint_data',
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=2800, shuffle=True, num_workers=0, 
    ...
    for batch in dataloader:
        ...
        print(batch['gt_joints'].min(dim=0)[0])
        print(batch['gt_joints'].max(dim=0)[0])
