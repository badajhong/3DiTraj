...
from genrobo3d.train.datasets.ik_mlp_dataset_test import (
    IK_MLP_Dataset, base_collate_fn
)

from genrobo3d.models.ik_mlp import IKMLP

DATASET_FACTORY = {
    'IKMLP': (IK_MLP_Dataset, base_collate_fn),
}
        ...
        for step, batch in enumerate(trn_dataloader):
            ...
            if (val_dataloader is not None) and (global_step % config.TRAIN.val_steps == 0):
                ...
                if default_gpu:
                    ...
                    # Add per-joint losses
                    for i in range(7):
                        joint_name = f"j{i+1}"
                        log_dict[f"val/{joint_name}_loss"] = val_metrics[joint_name]
                ...
                LOGGER.info('===============================================')
                if val_metrics['total_loss'] < best_val_metric:
                    best_val_metric = val_metrics['total_loss']
...
def validate(model, val_dataloader):
    model.eval()
    total_loss = 0
    joint_losses = {f'j{i+1}': 0.0 for i in range(7)}  # initialize per-joint loss accumulators
    num_batches = 0

    for batch in val_dataloader:
        pred_joints, loss = model(batch, compute_loss=True)
        pred_joints = pred_joints.cpu()

        total_loss += loss['total'].item()

        # Accumulate each joint's loss
        for i in range(7):
            joint_name = f'j{i+1}'
            joint_losses[joint_name] += loss[joint_name].item()

        num_batches += 1

    # Average the losses over number of batches
    averaged_joint_losses = {
        k: v / num_batches for k, v in joint_losses.items()
    }
...
if __name__ == '__main__':
    ...
    exp_config_path = "genrobo3d/configs/rlbench/ik_mlp.yaml"
    opts = [
    ]
    # ===============================================
