...
from genrobo3d.models.stage2_dit_joint_noncond import DiTraj
...
def prepare_padded_ground_truth(batch, batch_size, max_steps, robot_type, device):
    ...
    if gt_joints is not None and gt_actions is not None:
        ...
        for i in range(gt_actions.shape[0]):
    ...
    return pad_gt_traj.view(-1, 8), pad_mask.view(-1)
            ...
            if (val_dataloader is not None) and (global_step % config.TRAIN.val_steps == 0):
                ...
                if default_gpu:
                    wandb.log({
                        ...
                        "val/real_joint_loss": val_metrics['real_joint_loss'],
                        "step": global_step
                    })
...
def validate(model, val_dataloader):
    ...
    for batch in val_dataloader:
        ...
        pred_traj, loss = model.ddpm_sample(batch, 
                              pc_tokens,
                              final_pred_joints_actions,
                              max_steps=config.MODEL.action_config.max_steps, 
                              batch_size=config.TRAIN.train_batch_size, 
        ...
        real_joint_loss += loss['real_joint'].item()
        open_loss += loss['open'].item()
        total_loss += loss['total'].item() 

        num_examples += pred_traj.size(0)
        num_batches += 1
...
if __name__ == '__main__':
    ...
    exp_config_path = "genrobo3d/configs/rlbench/stage2_dit_joint_noncond.yaml"
    opts = [
    ]
    # ===============================================
