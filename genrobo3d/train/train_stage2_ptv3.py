...
from genrobo3d.models.stage2_ptv3 import (
    SimplePolicyPTV3AdaNorm, SimplePolicyPTV3CA #, SimplePolicyPTV3Concat
)


DATASET_FACTORY = {
    'SimplePolicyPTV3AdaNorm': (ptv3_dit_PolicyDataset, ptv3_collate_fn),
    'SimplePolicyPTV3CA': (ptv3_dit_PolicyDataset, ptv3_collate_fn),
    # 'SimplePolicyPTV3Concat': (ptv3_dit_PolicyDataset, ptv3_collate_fn),
}
...
def main(config):
    ...
    for epoch_id in range(restart_epoch, config.TRAIN.num_epochs):
        ...
        for step, batch in enumerate(trn_dataloader):
            # forward pass
            _, _, _, _, losses = model(batch, compute_loss=True, compute_final_action=False, global_step=global_step)
            ...
            for key, value in losses.items():
                ...
                if default_gpu:
                    wandb.log({f"loss/{key}": value.item(), "step": global_step})
            ...
            if (val_dataloader is not None) and (global_step % config.TRAIN.val_steps == 0):
                ...
                if default_gpu:
                    wandb.log({
                        ...
                        "val/open_loss": val_metrics['open_loss'],
                        "val/joint_loss": val_metrics['joint_loss'],
                        "step": global_step
                    })
...
def validate(model, val_dataloader):
    model.eval()
    pos_loss, rot_loss, open_loss, joint_loss, total_loss, num_examples, num_batches = 0, 0, 0, 0, 0, 0, 0
    pos_l1_loss, open_acc = 0, 0
    for batch in val_dataloader:
        pred_action, pred_joint, _, _, loss = model(batch, compute_loss=True, global_step=config.MODEL.num_train_steps)
        ...
        # pred_open = torch.sigmoid(pred_joint[..., -1]) > 0.5
        # open_acc += (pred_open == batch['gt_actions'][..., -1][end_indices].cpu()).float().sum().item()
        # pos_l1_loss += F.l1_loss(pred_action[..., :3], batch['gt_actions'][..., :3][end_indices].cpu())
...
if __name__ == '__main__':
    ...
    exp_config_path = "genrobo3d/configs/rlbench/stage2_ptv3.yaml"
    opts = [
    ]
    # ===============================================
