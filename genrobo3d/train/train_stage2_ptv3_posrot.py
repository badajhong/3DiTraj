...
from genrobo3d.models.stage2_ptv3_posrot import (
...
def main(config):
    ...
    if config.TRAIN.num_train_steps is None:
        ...
        LOGGER.info(f'#num_epoch: {config.TRAIN.num_epochs}')
    ...
    for epoch_id in range(restart_epoch, config.TRAIN.num_epochs):
        ...
        for step, batch in enumerate(trn_dataloader):
            # forward pass
            _, _, _, losses = model(batch, compute_loss=True, compute_final_action=False, global_step=global_step)
...
def validate(model, val_dataloader):
    ...
    for batch in val_dataloader:
        pred_action,  _, _, loss = model(batch, compute_loss=True, global_step=config.MODEL.loss_config.num_train_steps)
...
if __name__ == '__main__':
    ...
    exp_config_path = "genrobo3d/configs/rlbench/stage2_ptv3_posrot.yaml"
    opts = [
    ]
    # ===============================================
