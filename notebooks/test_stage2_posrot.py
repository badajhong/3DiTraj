...
from genrobo3d.models.stage2_ptv3_posrot import SimplePolicyPTV3CA
from genrobo3d.models.stage2_dit_posrot import DiTraj
from genrobo3d.configs.default import get_config

# PTV3 model is required to generate tokens
ptv3_ckpt_path = 'data/experiments/gembench/stage2_ptv3_posrot_0711/ckpts/model_step_5000.pt'
ptv3_cfg_path = "data/experiments/gembench/stage2_ptv3_posrot_0711/logs/training_config.yaml"

dit_ckpt_path = 'data/experiments/gembench/stage2_dit_posrot_0711/ckpts/model_step_50000.pt'
dit_cfg_path = "data/experiments/gembench/stage2_dit_posrot_0711/logs/training_config.yaml"
...
args = EasyDict({
    'exp_config': dit_cfg_path,
    'checkpoint': dit_ckpt_path,
    'device': device,
    'real_robot': False,
...
class SimpleActioner:
    def __init__(self, policy_model, ptv3_model, full_actioner, device='cuda'):
        self.policy = policy_model.ddpm_sample
    ...
    def predict(self, **kwargs):
        ...
        final_pred_joints_actions, pc_tokens, _ = self.ptv3(batch, compute_loss=False, global_step=None)
...
def normalize_action(action):
    ...
    # pos[0] = np.clip(pos[0], -0.27499998807907106, 0.774999988079071)
    # pos[1] = np.clip(pos[1], -0.6550000429153442, 0.654)
    # pos[2] = np.clip(pos[2], 0.752, 1.75)
    if not np.isclose(quat_norm, 1.0, atol=1e-3):
        quat = quat / quat_norm

    grip = (grip > 0.5).astype(int)
